####################################################################################
##### PHYLOGENETIC CLUSTERING SCRIPT - CLADE A ISOLATES ############################
####################################################################################
...
tree_file = "efm_dap_dataset_v1.cladeA.rmRCB.rmMGE.iqtree.contree.tree"
tree = read.tree(tree_file)
cat(paste("\t",length(tree$tip.label)," taxa found from phylogenetic tree\n",sep=""))
# 	639 taxa found from phylogenetic tree

# rooting on outgroup SRR6267866
tree = root(tree, which(tree$tip.label=="SRR6267866"))

laBS = as.numeric(tree$node.label);
tree2 = tree;
tree$node.label = paste("node",seq(1,tree$Nnode,1),sep="")

snp_file = "efm_dap_dataset_v1.cladeA.rmRCB.rmMGE.pairsnp.csv"
